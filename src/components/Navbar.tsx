 ...
 * تم حذف زر "أضف إعلانك" وحذف روابط "دليل سوف" و"المنصات التعليمية"،
 * والإبقاء على الروابط الأساسية للمنظومة: الرئيسية، سوق الوادي، سياحة وادنا، التطبيق، وشركاؤنا.
 */
...
export const Navbar: React.FC<NavbarProps> = ({ onNavigateSection }) => {
  ...
  const navLinks = [
    ...
    { label: 'سوق الوادي', href: '#ecosystem', externalUrl: 'https://souq.myeloued.com' },
    { label: 'سياحة وادنا', href: '#ecosystem', externalUrl: 'https://ouedna.myeloued.com/' },
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1 xl:gap-2">
            {navLinks.map((link) => (
              <a
                ...
                onClick={(e) => {
                  if (link.externalUrl) {
                    return; // allow default open or navigate
                  }
                  e.preventDefault();
                  onNavigateSection(link.href.replace('#', ''));
                }}
                className="px-3.5 py-2 text-sm font-medium text-white/80 hover:text-[#FBBF24] rounded-lg hover:bg-white/5 transition-colors"
              >
                {link.label}
              </a>
            ))}
          ...
          {/* Direct Portals Quick Jump & Mobile App Link (تم حذف زر أضف إعلانك) */}
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#160D07]/95 backdrop-blur-2xl border-b border-[#E5A93C]/30 px-5 pt-4 pb-6 mt-3 space-y-3 animate-in fade-in-50 duration-200">
